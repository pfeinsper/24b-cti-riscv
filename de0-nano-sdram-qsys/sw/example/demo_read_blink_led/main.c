...
int main() {
  ...
  while (1) {
    ...
    neorv32_cpu_delay_ms(5000); // wait 250ms using busy wait
    uint32_t ret = neorv32_gpio_pin_get(0);

    if (ret) {
      neorv32_uart0_printf("FIM DA PISCADA\n\n");
    } else {
      neorv32_uart0_printf("INICIO DA PISCADA\n\n");
    }
  }
